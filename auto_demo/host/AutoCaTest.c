...
static int  l_SecStorCa_Fs();
static void l_Aes_Test(CHAR* operMode, CHAR* aesMode);
static void l_Rsa_Test(CHAR* operFlag, EN_RSA_KEY_TYPE keyType, CHAR* output, UINT32 outputLen);
static void l_Rsa2048_Test(CHAR* operFlag, EN_RSA_KEY_TYPE keyType, CHAR* output, UINT32 outputLen);
...
*/
/****demo ==5  ***/
CHAR oldFileName[] = "COMMLOGIC_4_20200611_old.txt";
CHAR newFileName[] = "COMMLOGIC_4_20200611_new.txt";

CHAR readBuf[2560] = {0};
//CHAR writeBuf[] = "This is the test data which need be wrote into secure file";

CHAR writeBuf[] = "2020/06/11 09:00:01.327,Info,0,CCommLogic, Initialize, bDebug = False, machinePos = 4, startThead = False\
2020/06/11 09:00:01.467,Error,0,Close_LogTCP,isConnect=False,tcpCache.Count=0,clientSocket=True,machinePos=4,machineIP=192.168.250.74\
2020/06/11 09:00:01.467,Info,0,Connect.\
2020/06/11 09:00:01.497,Info,0,Serial Send: { 0xf001, 0x4001, 0x7e4, 0x6, 0xb, 0x9, 0x0, 0x1, 0x1d3, }2\
2020/06/11 09:00:01.527,Info,0,TcpData Received: { 0xC789, 0x4001, 0x0, } { 0xF002, 0x0001, [s]G_MCS_1.0.21_Build_200605_SZNo3, }\
2020/06/11 09:00:01.527,Info,0,收到指令，wCode=61442,iSeqNO=1 \
2020/06/11 09:00:01.527,Info,0,收到指令，wCode=61505,iSeqNO=2 \
2020/06/11 09:00:01.527,Info,0,收到指令，wCode=61760,iSeqNO=3\
2020/06/11 09:00:01.537,Info,0,Serial Send: { 0xb456, 0x1, 0x0, }\
2020/06/11 09:00:01.547,Info,0,TcpData Received: { 0xF042, 0x0004, 0xA, 0xF0010701, 0x0, 0xF0010401, 0x0, 0xF0010501, 0x0, 0xF0010801, 0x1, 0xF0010901, 0x0, 0xF0010A01, 0x0, },recverSlidingList.Count=3\
2020/06/11 09:00:01.547,Info,0,Serial Send: { 0xb456, 0x2, 0x0, }W\
2020/06/11 09:00:01.547,Info,0,收到指令，wCode=61506,iSeqNO=4\
2020/06/11 09:00:01.547,Info,0,收到指令，wCode=61506,iSeqNO=5\
2020/06/11 09:00:01.547,Info,0,收到指令，wCode=61506,iSeqNO=6\
2020/06/11 09:00:01.547,Info,0,收到指令，wCode=61506,iSeqNO=7\
2020/06/11 09:00:01.557,Info,0,Serial Send: { 0xb456, 0x3, 0x0, }X\
2020/06/11 09:00:01.567,Info,0,Serial Send: { 0xb456, 0x4, 0x0, }Y";
...
CHAR g_AesRawBuf[]    = "AutoMicroscopy IP 192.168.253.4 Amind PassWord!";
CHAR g_AesEcbRawBuf[] = "~~~~~You! You! check now, jian bing guo zi lai yi tao! yeah~~~~";

CHAR g_AesEncCbcBuf[] = 
{
	0x13, 0x3f, 0xca, 0xe4, 0xf1, 0xeb, 0x4e, 0xe1, 0xdf, 0xf0, 0x01, 0xb9, 0x44, 0xe1, 0xc7, 0x2d, 
	0x24, 0x73, 0x37, 0xe5, 0x85, 0xd1, 0x6e, 0x1a, 0x03, 0x7b, 0xfc, 0xc3, 0xb8, 0x20, 0xe0, 0xa6, 
	0x13, 0xaf, 0xf2, 0x40, 0xa8, 0xbb, 0xa9, 0x64, 0x3c, 0x06, 0xc4, 0xd1, 0x8d, 0x79, 0xf6, 0xdc
...
static int l_SecStorCa_Fs()
{
	...
	TF("read operation!\n");
	/** 2) Read data from secure file */
	l_Ret = g_SecStorCa_ReadFile(sizeof(oldFileName), oldFileName, 2000, readBuf);
	if(FAIL == l_Ret)
	{
		TF("Read secure file fail\n");
		return 0;
	...
	{
		//g_CA_PrintfBuffer(readBuf, 2000U);
		TF("The date mame is %s: \n",oldFileName);
		TF("The read data is:\n");
		TF("%s\n", readBuf);
	}
	...
	TF("read operation!\n");
	memset(readBuf, 0, 2000);
	/** 2) Read data from secure file */
	l_Ret = g_SecStorCa_ReadFile(sizeof(newFileName), newFileName, 2000, readBuf);
	if(FAIL == l_Ret)
	{
		TF("Read secure file fail\n");
		return 0;
	...
	{
		//g_CA_PrintfBuffer(readBuf, 2000);
		TF("The date mame is %s:\n",newFileName);
		TF("The read data is:\n");
		TF("%s\n", readBuf);
	}
	...
	l_Ret = g_SecStorCa_TrunCatFile(sizeof(newFileName), newFileName, 1000);
	if(FAIL == l_Ret)
	{
		TF("Write secure file fail\n");
		return 0;
	...
	TF("read3 operation!\n");
	memset(readBuf, 0, 2000);
	l_Ret = g_SecStorCa_ReadFile(sizeof(newFileName), newFileName, 1000U, readBuf);
	if(FAIL == l_Ret)
	{
		TF("Read secure file fail\n");
		return 0;
...
int main(int argc, char *argv[])
{
	...
	int l_Ret = FAIL;
	if(0 == memcmp(argv[1], "help", 4))
	{
		TF("\n");
		TF("optee_auto_demo		value		dec 		 \n");
		TF("optee_auto_demo 	value 		inc 	 	 \n");
		TF("\n");
		TF("optee_auto_demo 	fs 					 	 \n");
		TF("\n");
		TF("optee_auto_demo 	random 	    [len]    	 \n");
		TF("\n");  
		TF("optee_auto_demo 	sha1					 \n");
		TF("optee_auto_demo 	sha256  			 	 \n");
		TF("\n");
		TF("optee_auto_demo 	hmac  		[len] 	 	 [count]	\n");
		TF("\n");
		TF("optee_auto_demo		base64		enc			 \n");
		TF("optee_auto_demo		base64		dec		 	 \n");
		TF("\n");
		TF("optee_auto_demo 	aes  		enc 	 	cbc		\n");
		TF("optee_auto_demo 	aes  		enc 	 	ecb		\n");
		TF("optee_auto_demo 	aes  		enc 	 	ctr		\n");
		TF("optee_auto_demo 	aes  		enc 	 	cts		\n");
		TF("\n");
		TF("optee_auto_demo 	aes  		dec 	 	cbc		\n");
		TF("optee_auto_demo 	aes  		dec 	 	ecb		\n");
		TF("optee_auto_demo 	aes  		dec 	 	ctr		\n");
		TF("optee_auto_demo 	aes  		dec 	 	cts		\n");
		TF("\n");
		TF("optee_auto_demo 	pbkdf  		[len]		\n");
		TF("\n");
		TF("optee_auto_demo 	rsa1024  	sign 	 	\n");
		TF("optee_auto_demo 	rsa1024  	verify  	\n");
		TF("optee_auto_demo 	rsa1024  	enc 	 	\n");
		TF("optee_auto_demo 	rsa1024  	dec 	 	\n");
		TF("\n");
		TF("optee_auto_demo 	rsa2048  	sign 	 	\n");
		TF("optee_auto_demo 	rsa2048  	verify 	 	\n");
		TF("optee_auto_demo 	rsa2048  	enc 	 	\n");
		TF("optee_auto_demo 	rsa2048  	dec 	 	\n");		
	}
	
	if(0 == memcmp(argv[1], "value", 5))
	{
		if(0 == memcmp(argv[2], "dec", 3))
		{
			l_Ret = g_ValueCa_Dec(5229);
		}
		if(0 == memcmp(argv[2], "inc", 3))
		{
			l_Ret = g_ValueCa_Inc(5229);
		}
	}

	
	if(0 == memcmp(argv[1], "fs", 2))
	{
		 l_Ret = l_SecStorCa_Fs();
	}
    ...
    */
	...
	if(FAIL == l_Ret)
	{
		TF("Read secure file fail,the demo num is : %d\n", argv[1]);
		return 0;
	}
}
